...
`~/.rstack/analytics/` (user config directory, not project files). The skill
...
if [ "$_TEL" != "off" ]; then
  ...
  if [ -x ~/.rstack/skills/gstack/bin/gstack-telemetry-log ]; then
    ~/.rstack/skills/gstack/bin/gstack-telemetry-log \
...
\`\`\`bash
~/.rstack/skills/gstack/bin/gstack-review-read
\`\`\`

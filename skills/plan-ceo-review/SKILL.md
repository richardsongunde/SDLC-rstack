...
SLUG=$(~/.rstack/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
`~/.rstack/skills/gstack/office-hours/SKILL.md`
...
```bash
eval "$(~/.rstack/bin/rstack-slug 2>/dev/null)" && mkdir -p ~/.rstack/projects/$SLUG/ceo-plans
```
...
**Persist the result:**
```bash
~/.rstack/bin/rstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.rstack/skills/review/TODOS-format.md`.
...
```bash
~/.rstack/bin/rstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```

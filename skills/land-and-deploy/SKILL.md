...
- Did the canary check pass before marking the deploy healthy?
- Is rollback documented if health checks fail?
- Is the deploy URL verified in a browser (not just a curl)?
...
```bash
~/.rstack/bin/rstack-review-read 2>/dev/null
```
...
```bash
cat ~/.rstack/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(~/.rstack/bin/rstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)

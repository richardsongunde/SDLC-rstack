...
```bash
~/.rstack/bin/rstack-learnings-search --limit 5 --keyword "SKILL_NAME" 2>/dev/null || true
```
...
If yes, log it:
```bash
~/.rstack/bin/rstack-learnings-log '{"skill":"SKILL","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```
...
```bash
~/.rstack/bin/rstack-learnings-search --limit 5 --keyword "SKILL_NAME" 2>/dev/null
```
...
```bash
~/.rstack/bin/rstack-learnings-log '{"skill":"SKILL_NAME","type":"pitfall","key":"short-key","insight":"one sentence","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  ...
  Proactively suggest when the user says code is ready or asks about deploying. (rstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
- Did tests pass before the PR was created?
- Is the CHANGELOG entry user-facing language (not implementation details)?
- Was the version bumped and committed?
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.rstack/skills/review/TODOS-format.md`).
...
for PLAN_DIR in "$HOME/.rstack/projects/$_PLAN_SLUG" "$HOME/.rstack/plans" "$HOME/plans" ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
> rstack can search learnings from your other projects on this machine to find
...
This makes the compounding visible. The user should see that rstack is getting
...
1. Read `.rstack/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(~/.rstack/bin/rstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.rstack/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.rstack/bin/rstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
~/.rstack/bin/rstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
Read `.rstack/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(~/.rstack/bin/rstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.rstack/bin/rstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.rstack/bin/rstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.rstack/bin/rstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
Read `.rstack/skills/review/TODOS-format.md` for the canonical format reference.
...
Append to `~/.rstack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.rstack/projects/$SLUG/$BRANCH-reviews.jsonl
```

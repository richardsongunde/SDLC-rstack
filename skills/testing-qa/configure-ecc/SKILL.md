...
2. **Manual**: Copy only this skill to `~/.rstack/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
Options:
  - "User-level (~/.rstack/)" — "Applies to all your Codex projects"
  - "Project-level (.rstack/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.rstack`
- Project-level: `TARGET=.rstack` (relative to current project root)
- Both: `TARGET_USER=~/.rstack`, `TARGET_PROJECT=.rstack`
...
```bash
grep -rn "~/.rstack/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.rstack/` paths:
- If a skill references `~/.rstack/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.rstack/skills/` or `~/.rstack/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.rstack/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.rstack/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .rstack/skills/")
...
- For user-level: check `~/.rstack/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.rstack/skills/<skill-name>/SKILL.md` exists
...
- Some skills assume `~/.rstack/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.rstack/homunculus/` directory is always user-level — this is expected and not an error.

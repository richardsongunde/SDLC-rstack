...
[ -z "$_AUTO" ] && _AUTO=$(~/.rstack/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.rstack/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.rstack/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.rstack/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.rstack/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.rstack/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".rstack/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".rstack/skills/gstack"
elif [ -d "skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR="skills/gstack"
elif [ -d ".rstack/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".rstack/skills/gstack"
elif [ -d "$HOME/.rstack/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.rstack/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.rstack/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.rstack/skills/gstack"
  fi
fi
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.rstack/skills/gstack/` when you're ready."
...
```bash
~/.rstack/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
.rstack/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.rstack/skills/gstack/` when you're ready."

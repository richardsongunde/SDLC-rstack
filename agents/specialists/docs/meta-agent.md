...
2. **Read the format spec** — check the canonical format:
   ```bash
   cat skills/prompt-engineering/references/agent-format.md | head -50
   ```
...
If yes, log it:
```bash
rstack memory append '{"skill":"meta-agent","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

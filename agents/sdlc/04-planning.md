...
## Skills to load:
```bash
cat skills/plan-ceo-review/SKILL.md | head -30
...
If yes, log it:
```bash
rstack memory append '{"skill":"04-planning","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

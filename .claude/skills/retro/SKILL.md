...
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/bin/rstack-global-discover ] && DISCOVER_BIN=~/.claude/bin/rstack-global-discover

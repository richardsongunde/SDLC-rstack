...
~/.claude/bin/rstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
```bash
eval "$(~/.claude/bin/rstack-slug 2>/dev/null)"
~/.claude/bin/rstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claude/bin/rstack-slug 2>/dev/null)"
~/.claude/bin/rstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.claude/bin/rstack-slug 2>/dev/null)"
~/.claude/bin/rstack-learnings-search --limit 50 2>/dev/null
```
...
```bash
eval "$(~/.claude/bin/rstack-slug 2>/dev/null)"
GSTACK_HOME="${GSTACK_HOME:-$HOME/.rstack}"
...
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```

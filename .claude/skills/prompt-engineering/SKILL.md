...
description: |
  Wizard for creating well-formatted Claude Code agents, skills, plugins, and hooks
...
4. **Write it to the correct path:**
   - Agent → `.claude/agents/specialists/[backend|frontend|devops|security|data|qa|product|docs|crypto]/[name].md`
   - Skill → `.claude/skills/[name]/SKILL.md`
   - Plugin agent → `.claude/plugins/[plugin]/agents/[name].md`
   - Hook → `.claude/hooks/scripts/[name].py`

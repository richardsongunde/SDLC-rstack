...
```bash
_UPD=$(~/.claude/bin/rstack-update-check 2>/dev/null || .claude/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
...
_CONTRIB=$(~/.claude/bin/rstack-config get gstack_contributor 2>/dev/null || true)
...
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(~/.claude/bin/rstack-config get skill_prefix 2>/dev/null || echo "false")
...
for _PF in $(find ~/.rstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "~/.claude/bin/rstack-telemetry-log" ]; then
      ~/.claude/bin/rstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
If A: run `~/.claude/bin/rstack-config set telemetry community`
...
If B→A: run `~/.claude/bin/rstack-config set telemetry anonymous`
If B→B: run `~/.claude/bin/rstack-config set telemetry off`
...
If A: run `~/.claude/bin/rstack-config set proactive true`
If B: run `~/.claude/bin/rstack-config set proactive false`
...
If B: run `~/.claude/bin/rstack-config set routing_declined true` Say "No problem. You can add routing rules later by running `rstack-config set routing_declined false` and re-running any skill."
...
if [ "$_TEL" != "off" ]; then
  ...
  if [ -x ~/.claude/bin/rstack-telemetry-log ]; then
    ~/.claude/bin/rstack-telemetry-log \

...
Builder  writes → outputs/team_state/[task].json
                  Required fields: files_modified, tests_run, status

Validator reads → outputs/team_state/[task]_validation.json
                  Required fields: checks[], status (PASS/FAIL), issues[]
```
...
Skills are prompt templates read by Claude, not bash scripts.

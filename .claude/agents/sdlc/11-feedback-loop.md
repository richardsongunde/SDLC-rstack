...
## Skill to load:
```bash
cat .claude/skills/retro/SKILL.md | head -30
```
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"11-feedback-loop","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

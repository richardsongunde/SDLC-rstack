...
Follow `.claude/agents/OPERATING-STANDARD.md` for every run. Key rules: verify before acting, keep context lean, ask one focused question when requirements are ambiguous, prefer `.rstack/runs/<run_id>/` over legacy `outputs/team_state/`, write the required builder/validator contract, and never report DONE without evidence.
...
**Before starting:** Read `.claude/agents/OPERATING-STANDARD.md`, the assigned task packet, and the relevant skill. State the exact scope, files likely touched, and verification command in 2 sentences. If any acceptance criterion is unclear, return `NEEDS_CONTEXT` instead of guessing.
...
```bash
cat .claude/plugins/[plugin-name]/agents/[agent-name].md
```

To use a plugin skill:
```bash
cat .claude/plugins/[plugin-name]/skills/[skill-name]/SKILL.md
```
...
cat .claude/skills/investigate/SKILL.md    # if debugging
cat .claude/skills/careful/SKILL.md        # if touching prod or destructive ops
# Example: UI task
...
cat .claude/plugins/payment-processing/skills/stripe-integration/SKILL.md
```
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"builder","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

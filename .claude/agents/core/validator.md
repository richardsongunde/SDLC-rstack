...
hooks:
  SessionEnd:
    - hooks:
        - type: command
          command: >-
            uv run $CLAUDE_PROJECT_DIR/.claude/hooks/scripts/post_agent_contract_validator.py
---
...
**Before starting:** Read `.claude/agents/OPERATING-STANDARD.md`, the task prompt, and `$RSTACK_RUN_DIR/tasks/<task_id>/builder.json`. State in 2 sentences whether you will validate directly or delegate to specialist reviewers. If evidence is insufficient, return `FAIL` or `NEEDS_CONTEXT`, never a weak PASS.
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"validator","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

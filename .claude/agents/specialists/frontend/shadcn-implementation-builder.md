...
description: |
  Specialist agent for shadcn implementation builder tasks and workflows. Trigger:
  any request involving shadcn implementation builder. (frontend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"shadcn-implementation-builder","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

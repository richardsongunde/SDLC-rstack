...
description: |
  Specialist agent for shadcn requirements analyzer tasks and workflows. Trigger:
  any request involving shadcn requirements analyzer. (frontend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"shadcn-requirements-analyzer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Specialist agent for shadcn quick helper tasks and workflows. Trigger: any
  request involving shadcn quick helper. (frontend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"shadcn-quick-helper","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

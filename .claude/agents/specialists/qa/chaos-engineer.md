...
name: chaos-engineer
description: |
  Specialist agent for chaos engineer tasks and workflows. Trigger: any request
  involving chaos engineer. (qa)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"chaos-engineer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

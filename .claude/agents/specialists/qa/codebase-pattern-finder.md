...
description: |
  Specialist agent for codebase pattern finder tasks and workflows. Trigger: any
  request involving codebase pattern finder. (qa)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"codebase-pattern-finder","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

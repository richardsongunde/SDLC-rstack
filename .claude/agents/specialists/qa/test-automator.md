...
name: test-automator
description: |
  Sets up test infrastructure and writes automated unit/integration tests.
  Improves coverage systematically. Trigger: "write tests for [function/module]",
  "set up testing for [project]", "add test coverage for [code path]". (qa)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"test-automator","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Specialist agent for cs quality regulatory tasks and workflows. Trigger: any
  request involving cs quality regulatory. (qa)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"cs-quality-regulatory","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

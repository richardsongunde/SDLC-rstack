...
description: |
  Specialist agent for performance profiler tasks and workflows. Trigger: any
  request involving performance profiler. (qa)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"performance-profiler","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

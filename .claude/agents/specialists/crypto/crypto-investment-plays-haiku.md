...
description: |
  Specialist agent for crypto investment plays haiku tasks and workflows. Trigger:
  any request involving crypto investment plays haiku. (crypto)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"crypto-investment-plays-haiku","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Specialist agent for crypto market agent haiku tasks and workflows. Trigger: any
  request involving crypto market agent haiku. (crypto)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"crypto-market-agent-haiku","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

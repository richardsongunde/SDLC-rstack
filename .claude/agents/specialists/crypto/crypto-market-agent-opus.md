...
description: |
  Specialist agent for crypto market agent opus tasks and workflows. Trigger: any
  request involving crypto market agent opus. (crypto)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"crypto-market-agent-opus","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

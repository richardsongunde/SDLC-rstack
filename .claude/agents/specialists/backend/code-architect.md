...
name: code-architect
description: |
  Reviews internal code design for SOLID violations, circular dependencies, and
  missing abstractions. Proposes refactoring plans. Trigger: "design the code
  architecture", "review this code structure", "suggest a refactoring plan".
  (backend)
model: sonnet
tools:
  - Bash
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"code-architect","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

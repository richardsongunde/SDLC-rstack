...
name: cpp-pro
description: |
  Specialist agent for cpp pro tasks and workflows. Trigger: any request involving
  cpp pro. (backend)
model: sonnet
tools:
  - Bash
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"cpp-pro","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

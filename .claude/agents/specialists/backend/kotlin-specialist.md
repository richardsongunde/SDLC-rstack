...
description: |
  Specialist agent for kotlin specialist tasks and workflows. Trigger: any request
  involving kotlin specialist. (backend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"kotlin-specialist","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

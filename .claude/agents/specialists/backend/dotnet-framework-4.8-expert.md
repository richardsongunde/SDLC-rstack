...
description: |
  Specialist agent for dotnet framework 4.8 expert tasks and workflows. Trigger:
  any request involving dotnet framework 4.8 expert. (backend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"dotnet-framework-4.8-expert","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

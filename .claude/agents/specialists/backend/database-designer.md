...
description: |
  Creates and modifies specific database tables, columns, and migrations. Writes
  up/down migration files. Trigger: "create the schema for [tables]", "add a
  column to [table]", "write a migration for [change]". (backend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"database-designer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

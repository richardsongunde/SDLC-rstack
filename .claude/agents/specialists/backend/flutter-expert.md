...
name: flutter-expert
description: |
  Specialist agent for flutter expert tasks and workflows. Trigger: any request
  involving flutter expert. (backend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"flutter-expert","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

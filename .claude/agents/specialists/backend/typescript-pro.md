...
name: typescript-pro
description: |
  Specialist agent for typescript pro tasks and workflows. Trigger: any request
  involving typescript pro. (backend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"typescript-pro","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Decomposes monoliths and designs microservice boundaries using DDD bounded
  contexts. Defines inter-service communication (sync/async). Trigger: "break up
  this monolith", "design service boundaries", "plan event-driven architecture".
  (backend)
model: sonnet
tools:
  - Bash
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"microservices-architect","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

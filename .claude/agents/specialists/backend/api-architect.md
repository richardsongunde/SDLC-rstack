...
name: api-architect
description: |
  Designs service-level API architectures, defines inter-service contracts, and
  sets API governance standards. Use when setting service boundaries, reviewing
  API coupling, or designing API gateway patterns. Trigger: "design the service
  architecture", "review API boundaries", "plan inter-service communication".
  (backend)
model: sonnet
tools:
  - Bash
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"api-architect","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
name: rails-expert
description: |
  Specialist agent for rails expert tasks and workflows. Trigger: any request
  involving rails expert. (backend)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"rails-expert","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

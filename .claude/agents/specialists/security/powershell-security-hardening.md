...
description: |
  Specialist agent for powershell security hardening tasks and workflows. Trigger:
  any request involving powershell security hardening. (security)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"powershell-security-hardening","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

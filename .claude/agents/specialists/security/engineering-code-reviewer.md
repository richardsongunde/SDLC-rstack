...
description: |
  Specialist agent for engineering code reviewer tasks and workflows. Trigger: any
  request involving engineering code reviewer. (security)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"engineering-code-reviewer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

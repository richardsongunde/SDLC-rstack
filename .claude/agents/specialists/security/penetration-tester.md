...
description: |
  Authorized penetration testing: tests auth bypass, injection, IDOR, and other
  exploit vectors. Trigger: "test [application] for vulnerabilities", "perform a
  pentest", "verify [auth] can be bypassed". (security)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"penetration-tester","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

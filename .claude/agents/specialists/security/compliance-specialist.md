...
description: |
  Specialist agent for compliance specialist tasks and workflows. Trigger: any
  request involving compliance specialist. (security)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"compliance-specialist","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

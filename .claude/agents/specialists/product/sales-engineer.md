...
name: sales-engineer
description: |
  Specialist agent for sales engineer tasks and workflows. Trigger: any request
  involving sales engineer. (product)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"sales-engineer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

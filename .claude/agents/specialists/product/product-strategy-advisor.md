...
description: |
  Specialist agent for product strategy advisor tasks and workflows. Trigger: any
  request involving product strategy advisor. (product)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"product-strategy-advisor","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

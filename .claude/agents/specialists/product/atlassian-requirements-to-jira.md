...
description: |
  Specialist agent for atlassian requirements to jira tasks and workflows.
  Trigger: any request involving atlassian requirements to jira. (product)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"atlassian-requirements-to-jira","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
name: trend-analyst
description: |
  Specialist agent for trend analyst tasks and workflows. Trigger: any request
  involving trend analyst. (product)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"trend-analyst","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Specialist agent for product sprint prioritizer tasks and workflows. Trigger:
  any request involving product sprint prioritizer. (product)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"product-sprint-prioritizer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

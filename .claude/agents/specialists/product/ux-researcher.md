...
name: ux-researcher
description: |
  Specialist agent for ux researcher tasks and workflows. Trigger: any request
  involving ux researcher. (product)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"ux-researcher","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

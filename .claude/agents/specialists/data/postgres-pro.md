...
name: postgres-pro
description: |
  Specialist agent for postgres pro tasks and workflows. Trigger: any request
  involving postgres pro. (data)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"postgres-pro","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

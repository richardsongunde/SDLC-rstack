...
description: |
  Specialist agent for work completion summary tasks and workflows. Trigger: any
  request involving work completion summary. (docs)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"work-completion-summary","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
name: legal-advisor
description: |
  Specialist agent for legal advisor tasks and workflows. Trigger: any request
  involving legal advisor. (docs)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"legal-advisor","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

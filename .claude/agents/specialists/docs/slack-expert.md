...
name: slack-expert
description: |
  Specialist agent for slack expert tasks and workflows. Trigger: any request
  involving slack expert. (docs)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"slack-expert","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
name: api-documenter
description: |
  Specialist agent for api documenter tasks and workflows. Trigger: any request
  involving api documenter. (docs)
model: sonnet
tools:
  - Read
  - Write
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"api-documenter","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

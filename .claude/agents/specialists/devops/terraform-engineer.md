...
description: |
  Infrastructure-as-code with Terraform/OpenTofu. Writes, validates, and refactors
  Terraform modules. Trigger: "write Terraform for [resource]", "review this
  Terraform", "refactor to use modules". (devops)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"terraform-engineer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

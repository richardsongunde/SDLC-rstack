...
description: |
  Specialist agent for knowledge synthesizer tasks and workflows. Trigger: any
  request involving knowledge synthesizer. (devops)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"knowledge-synthesizer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Specialist agent for it ops orchestrator tasks and workflows. Trigger: any
  request involving it ops orchestrator. (devops)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"it-ops-orchestrator","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

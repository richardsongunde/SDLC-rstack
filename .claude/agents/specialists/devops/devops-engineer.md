...
description: |
  CI/CD pipelines, deployment automation, and build orchestration. Trigger: "set
  up CI/CD", "add a GitHub Actions pipeline", "automate [deployment/build/test]
  process". (devops)
model: sonnet
tools:
  - Bash
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"devops-engineer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

...
description: |
  Specialist agent for deployment engineer tasks and workflows. Trigger: any
  request involving deployment engineer. (devops)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"deployment-engineer","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

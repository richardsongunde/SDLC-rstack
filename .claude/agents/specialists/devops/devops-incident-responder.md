...
description: |
  Specialist agent for devops incident responder tasks and workflows. Trigger: any
  request involving devops incident responder. (devops)
model: sonnet
tools:
  - Bash
  - Read
...
If yes, log it:
```bash
~/.claude/bin/rstack-learnings-log '{"skill":"devops-incident-responder","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":8,"source":"observed"}' 2>/dev/null || true
```

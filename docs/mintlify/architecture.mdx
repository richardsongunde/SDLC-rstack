...
| Layer | What it does |
|---|---|
| **Runtime Adapter** | Binds RStack to Pi via lifecycle hooks and registers 15 SDLC tools |
| **Pipeline Engine** | Routes work through 15 canonical SDLC stages, each producing a typed artifact |
| **Harness Core** | Enforces contracts, validates evidence, manages run state |
| **Governance** | Approval gates, guardrails, and human-in-the-loop controls |
| **Security** | Protected actions, secret scanning, prompt injection detection |
| **Episodic Memory** | Cross-run learning with lexical retrieval and validator-gated writes |
| **Asset Registry** | 196 agents · 156 skills · 72 plugins · 36 prompts, fully indexed |
...
<Note>
  **Excalidraw source** — the full editable diagram is at `docs/mintlify/assets/rstack-ecosystem-architecture.excalidraw`. Open it at [excalidraw.com](https://excalidraw.com) (File → Open) or in any Excalidraw-compatible editor. It covers all eight layers: Developer trigger → Pi Extension → Harness Core → 15-Stage Pipeline → Governance → Security → Episodic Memory → Asset Registry → Run State.
</Note>
